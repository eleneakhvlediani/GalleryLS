...
public protocol PlayVideoDelegate {
    func initVideo(player:AVPlayer?, playerLayer:AVPlayerLayer?)
  //  func startPlaying()
}


open class INSPhotosViewController: UIViewController, UIPageViewControllerDataSource, UIPageViewControllerDelegate, UIViewControllerTransitioningDelegate, PlayVideoDelegate {
    ...
    public func initVideo(player:AVPlayer?, playerLayer:AVPlayerLayer?) {
        
       // currentPhotoViewController?.scalingImageView.initializeVideo(player: player, playerLayer: playerLayer, viewController: self)
        avplayer =  currentPhotoViewController?.scalingImageView.player
       
        avplayerLayer = playerLayer
        
        if avplayer == nil && avplayerLayer != nil {
            avplayer = avplayerLayer?.player
        }
        
    }
    
//    public func startPlaying(){
//        currentPhotoViewController?.scalingImageView.playVideo()
//        
//    }

    
//    public func startPlaying(player:AVPlayer?) {
//        currentPhotoViewController?.scalingImageView.initializeVideo(player: player, viewController: self)
//        
//    }
    ...
    public init(photos: [INSPhotoViewable], initialPhoto: INSPhotoViewable? = nil, referenceView: UIView? = nil, frame:CGRect? = nil) {
        dataSource = INSPhotosDataSource(photos: photos)
        
        super.init(nibName: nil, bundle: nil)

        self.passedFrame = frame
        
        avplayerLayer = initialPhoto?.playerLayer
        
        initialSetupWithInitialPhoto(initialPhoto)
        
        
        transitionAnimator.startingView = referenceView
        if initialPhoto?.isVideo == 1 {
           // transitionAnimator.endingView = currentPhotoViewController?.scalingImageView.imageView
            
            DispatchQueue.main.async {
                self.initVideo(player: initialPhoto?.video, playerLayer:initialPhoto?.playerLayer)
            }
    ...
    private func initialSetupWithInitialPhoto(_ initialPhoto: INSPhotoViewable? = nil) {
        transitionAnimator.playVideoDelegate = self
        overlayView.photosViewController = self
    ...
    open var videoIsOnItsPlace:Bool = false
    
    open override func viewWillAppear(_ animated: Bool) {
        
        if(currentPhoto?.isVideo == 1 && avplayerLayer != nil && videoIsOnItsPlace == false){
            
            videoIsOnItsPlace = true
            
            CATransaction.begin()
            CATransaction.setDisableActions(true)
            
            avplayerLayer?.frame = passedFrame!
            
            let animationDuration:CGFloat = 0.2
            CATransaction.commit()
            
            
            let lastPosition = self.view.center.y - (self.avplayerLayer?.frame.height)! / 2
            let difference = ((lastPosition - (passedFrame?.origin.y)!) / lastPosition)
            
            var animationDurationForThisFrame = difference * animationDuration
            if animationDurationForThisFrame < 0 {
                animationDurationForThisFrame *= -1
                
            }
            
            if animationDurationForThisFrame < 0.05 {
                animationDurationForThisFrame = 0.1
            }
            ...
            animation.fromValue = self.avplayerLayer?.position
            animation.toValue = self.view.center
            animation.duration = CFTimeInterval(Double(animationDurationForThisFrame))
            
            self.avplayerLayer?.position = self.view.center
            
            self.avplayerLayer?.add(animation, forKey: "position")
            CATransaction.commit()
            ...
            if self.avplayerLayer?.player?.isPlaying == false {
                
                
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                    self.avplayerLayer?.player?.play()
                    (self.currentPhoto as! INSPhoto).playerLayer = nil
    ...
    private func updateCurrentPhotosInformation() {
        if let currentPhoto = currentPhoto {
            ...
            if(currentPhoto.isVideo == 1){
                initVideo(player: currentPhoto.video, playerLayer: avplayerLayer)
    ...
    open override func dismiss(animated flag: Bool, completion: (() -> Void)?) {
        ...
        super.dismiss(animated: flag) { () -> Void in
            ...
            if !isStillOnscreen {
              ...
              self.stopVideo(photo: self.currentPhoto)
                self.currentPhotoViewController?.scalingImageView.removeVideo()

            }
            completion?()
        }
    }
    func stopVideo(photo:INSPhotoViewable?) {
        
        if photo?.isVideo == 1 {
            photo?.video?.pause()
            photo?.video?.seek(to: kCMTimeZero)
        }
//        avplayer?.pause()
//        avplayer?.seek(to: kCMTimeZero)
        
        //currentPhotoViewController?.playerLayer?.player?.pause()
       // currentPhotoViewController?.playerLayer = nil
    ...
    private func initializePhotoViewControllerForPhoto(_ photo: INSPhotoViewable) -> INSPhotoViewController {
        let photoViewController = INSPhotoViewController(photo: photo, playerLayer: photo.playerLayer)
    ...
    @objc open func pageViewController(_ pageViewController: UIPageViewController, didFinishAnimating finished: Bool, previousViewControllers: [UIViewController], transitionCompleted completed: Bool) {
        if completed {
            let prevViewController  = previousViewControllers.last as! INSPhotoViewController
            self.stopVideo(photo: prevViewController.photo)
