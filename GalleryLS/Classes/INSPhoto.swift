...
@objc public protocol INSPhotoViewable: class {
    ...
    var video: AVPlayer? { get }
    var isVideo: NSNumber? { get }
  ...
  //  var playerViewController: AVPlayerViewController? { get }
    ...
    func loadVideoWithCompletionHandler(_ completion: @escaping (_ video: AVPlayer?, _ error: Error?) -> ())
...
open class INSPhoto: INSPhotoViewable, Equatable {
   ...
   // public var playerViewController:AVPlayerViewController?
    ...
    public init(image:UIImage?, video: AVPlayer?, thumbnailImage: UIImage?) {
        self.video = video
        self.image = image
        self.thumbnailImage = thumbnailImage
        self.isVideo = 1
    ...
    public init(video: AVPlayer?, thumbnailImageURL: URL?) {
        self.video = video
    ...
    @objc open func loadImageWithCompletionHandler(_ completion: @escaping (_ image: UIImage?, _ error: Error?) -> ()) {
        ...
        loadImageWithURL(url, completion: completion)
    }
    
    
    @objc open func loadVideoWithCompletionHandler(_ completion: @escaping (_ video: AVPlayer?, _ error: Error?) -> ()) {
        if let video = video {
            completion(video, nil)
            return
        }
        loadVideoWithURL(url, completion: completion)
    }
    ...
    open func loadVideoWithURL(_ url: URL?, completion: @escaping (_ video: AVPlayer?, _ error: Error?) -> ()) {
        let session = URLSession(configuration: URLSessionConfiguration.default)
