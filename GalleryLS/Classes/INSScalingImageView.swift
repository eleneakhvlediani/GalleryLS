...
//        playerLayer?.removeFromSuperlayer()
    ...
    func playVideo(){
        player?.seek(to: kCMTimeZero)
        player?.play()
//        avPlayerController.p
    }
    
//    func initializeVideo(player:AVPlayer?,playerLayer:AVPlayerLayer? = nil, viewController:UIViewController?) {
//        videoView = UIView(frame: imageView.frame)
...
//    }
    ...
    var video:AVPlayer? {
        didSet {
            updateVideo(video, thumbnailImage: thumbnailImage)
        }
    }
    ...
    private func updateVideo(_ video: AVPlayer?, thumbnailImage:UIImage?) {
        let size = thumbnailImage?.size ?? CGSize.zero
        
        
        videoView?.transform = CGAffineTransform.identity
        //video = video
        videoView?.frame = CGRect(origin: CGPoint.zero, size: size)
        self.contentSize = size
        
        updateZoomScaleVideo()
        centerScrollViewContents()
    }
    ...
    private func updateZoomScaleVideo() {
        if videoView != nil {
            let scrollViewFrame = self.bounds
            let scaleWidth = scrollViewFrame.size.width / (videoView?.frame.width)!
            let scaleHeight = scrollViewFrame.size.height / (videoView?.frame.height)!
            let minimumScale = min(scaleWidth, scaleHeight)
